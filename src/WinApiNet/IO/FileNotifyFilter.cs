namespace WinApiNet.IO
{
    ...
    /// <summary>
    /// File notification filter flags used by <see cref="WinDirectory"/>.
    /// </summary>
    [Flags]
    public enum FileNotifyFilter : uint
    {
        /// <summary>
        /// Empty notification filter.
        /// </summary>
        NONE = 0x00000000,

        /// <summary>
        /// Any file name change in the watched directory or subtree causes a change notification wait operation to
        /// return. Changes include renaming, creating, or deleting a file name.
        /// </summary>
        ...
        /// wait operation to return. The operating system detects a change to the last write-time only when the file
        /// is written to the disk. For operating systems that use extensive caching, detection occurs only when the
